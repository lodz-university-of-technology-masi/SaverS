...
function getJson() {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: `https://x3vqos9dhc.execute-api.us-east-1.amazonaws.com/TestAPIv2/attribution/candidate/${getUserName()}`,
      type: "GET",
      headers: {
        Authorization: getToken(),
        "Content-Type": "application/json",
      ...
      success: data => {
        console.log(data);
        dataattribution = JSON.parse(data.body);
        return resolve();
        // updateTodoList();
      },
      error: err => {
...
$(document).ready(function() {
  getJson().then(() => {
    console.log(dataattribution);
    const app = document.getElementById("main");
    for (let j in dataattribution.attributions) {
      let i = dataattribution.attributions[j]
  ...
  //  if(i.resolved=="true")
  //   {
  //   button.classList.add("btn", "btn-outline-info", "btn-lg", "btn-block");
